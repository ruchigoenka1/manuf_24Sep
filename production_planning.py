...
def render_gantt_charts(df):
    ...
    base_colors = [
        '#8CD4FF', '#0673DF', '#FFB4B4', '#FF2B2B', 
        '#83E883', '#FFA020', '#B05BFF', '#FFCC00'
    ]
...
if st.button(f"🚀 Run {solver_choice}", type="primary"):
    ...
    if solver_choice == "Optimizer":
        ...
        with st.spinner("Evolving demand schedule..."):
            res = minimize(
                DemandGA(base_tasks, start_day_dict, deadline_dict, df_changeover, scheduling_strategy), 
                GA(pop_size=ga_pop_size), 
                get_termination("n_gen", ga_generations), 
                seed=1
            )
